...
void ImGui_ImplVulkan_RenderDrawData( ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline )
{
	...
	if ( wrb->FrameRenderBuffers == nullptr )
	{
		...
		wrb->FrameRenderBuffers = (ImGui_ImplVulkanH_FrameRenderBuffers*)
			IM_ALLOC( sizeof( ImGui_ImplVulkanH_FrameRenderBuffers ) * wrb->Count );
		...
		for ( int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++ )
		{
			...
			else
			{
				...
				vkCmdBindDescriptorSets(
					command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, bd->PipelineLayout, 0, 1, desc_set, 0, nullptr );
...
bool ImGui_ImplVulkan_CreateFontsTexture()
{
	...
	bd->FontDescriptorSet = (VkDescriptorSet)
	...
	// Copy to Image:
	{
		...
		vkCmdCopyBufferToImage(
			bd->FontCommandBuffer, upload_buffer, bd->FontImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region );
...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
	...
	ImGui_ImplVulkan_CreatePipeline(
		v->Device, v->Allocator, v->PipelineCache, bd->RenderPass, v->MSAASamples, &bd->Pipeline, bd->Subpass );
...
bool ImGui_ImplVulkan_LoadFunctions(
	PFN_vkVoidFunction ( *loader_func )( const char* function_name, void* user_data ), void* user_data )
{
...
#undef IMGUI_VULKAN_FUNC_LOAD
	...
	ImGuiImplVulkanFuncs_vkCmdBeginRenderingKHR =
		reinterpret_cast< PFN_vkCmdBeginRenderingKHR >( loader_func( "vkCmdBeginRenderingKHR", user_data ) );
...
bool ImGui_ImplVulkan_Init( ImGui_ImplVulkan_InitInfo* info, VkRenderPass render_pass )
{
	...
	if ( info->UseDynamicRendering )
	{
		...
		ImGuiImplVulkanFuncs_vkCmdBeginRenderingKHR = reinterpret_cast<
			PFN_vkCmdBeginRenderingKHR >( vkGetInstanceProcAddr( info->Instance, "vkCmdBeginRenderingKHR" ) );
		ImGuiImplVulkanFuncs_vkCmdEndRenderingKHR = reinterpret_cast<
			PFN_vkCmdEndRenderingKHR >( vkGetInstanceProcAddr( info->Instance, "vkCmdEndRenderingKHR" ) );
#endif
		IM_ASSERT( ImGuiImplVulkanFuncs_vkCmdBeginRenderingKHR != nullptr );
		IM_ASSERT( ImGuiImplVulkanFuncs_vkCmdEndRenderingKHR != nullptr );
#else
		IM_ASSERT(
			0 && "Can't use dynamic rendering when neither VK_VERSION_1_3 or VK_KHR_dynamic_rendering is defined." );
#endif
	}
	...
	if ( info->UseDynamicRendering == false ) IM_ASSERT( render_pass != VK_NULL_HANDLE );
...
void ImGui_ImplVulkan_NewFrame()
{
	...
	if ( !bd->FontDescriptorSet ) ImGui_ImplVulkan_CreateFontsTexture();
}
	...
	uint32_t min_image_count )
{
	...
	if ( min_image_count == 0 ) min_image_count = ImGui_ImplVulkanH_GetMinImageCountFromPresentMode( wd->PresentMode );
	...
	// Create Swapchain
	{
		...
		wd->FrameSemaphores = (ImGui_ImplVulkanH_FrameSemaphores*)
			IM_ALLOC( sizeof( ImGui_ImplVulkanH_FrameSemaphores ) * wd->ImageCount );
