...
inline ImGuiTableFlags TableFixFlags( ImGuiTableFlags flags, ImGuiWindow* outer_window )
{
	...
	if ( ( flags & ImGuiTableFlags_SizingMask_ ) == 0 )
		flags |=
			( ( flags & ImGuiTableFlags_ScrollX ) || ( outer_window->Flags & ImGuiWindowFlags_AlwaysAutoResize ) ) ?
	...
	if ( outer_window->RootWindow->Flags & ImGuiWindowFlags_NoSavedSettings ) flags |= ImGuiTableFlags_NoSavedSettings;
...
bool ImGui::BeginTable(
	const char* str_id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width )
{
	...
	float inner_width )
{
	...
	if ( use_child_window )
	{
		...
		if ( ( flags & ImGuiTableFlags_ScrollX ) && inner_width > 0.0f ) override_content_size.x = inner_width;
		...
		IM_ASSERT(
			table->InnerWindow->WindowPadding.x == 0.0f && table->InnerWindow->WindowPadding.y == 0.0f
	...
	if ( instance_no > 0 )
		PushOverrideID(
			instance_id ); // FIXME: Somehow this is not resolved by stack-tool, even tho GetIDWithSeed() submitted the symbol.
...
static void TableSetupColumnFlags( ImGuiTable* table, ImGuiTableColumn* column, ImGuiTableColumnFlags flags_in )
{
	...
	{
		IM_ASSERT(
			ImIsPowerOfTwo( flags & ImGuiTableColumnFlags_WidthMask_ ) ); // Check that only 1 of each set is used.
	}
...
void ImGui::TableUpdateLayout( ImGuiTable* table )
{
	...
	for ( int order_n = 0; order_n < table->ColumnsCount; order_n++ )
	{
		...
		if ( column->SortOrder > 0 && !( table->Flags & ImGuiTableFlags_SortMulti ) ) table->IsSortSpecsDirty = true;
		...
		if ( !column->IsPreserveWidthAuto ) column->WidthAuto = TableGetColumnWidthAuto( table, column );
	...
	for ( int column_n = 0; column_n < table->ColumnsCount; column_n++ )
	{
		...
		if ( column->Flags & ImGuiTableColumnFlags_WidthFixed )
		{
			...
			if ( column->AutoFitQueue != 0x00 )
				...
				( column->Flags & ImGuiTableColumnFlags_WidthFixed ) && !column_is_resizable
	...
	if ( width_remaining_for_stretched_columns >= 1.0f && !( table->Flags & ImGuiTableFlags_PreciseWidths ) )
		for ( int order_n = table->ColumnsCount - 1;
		      stretch_sum_weights > 0.0f && width_remaining_for_stretched_columns >= 1.0f && order_n >= 0;
		      order_n-- )
		{
	...
	if ( table->AngledHeadersHeight > 0.0f )
		if ( g.IO.MousePos.y >= table->OuterRect.Min.y
			...
			mouse_skewed_x += ImTrunc(
				( table->OuterRect.Min.y + table->AngledHeadersHeight - g.IO.MousePos.y ) * table->AngledHeadersSlope );
	...
	if ( is_hovering_table && table->HoveredColumnBody == -1 )
		if ( mouse_skewed_x >= unused_x1 ) table->HoveredColumnBody = (ImGuiTableColumnIdx)table->ColumnsCount;
	...
	if ( table->IsContextPopupOpen && table->ContextPopupColumn != -1
		...
		( table->Flags & ImGuiTableFlags_HighlightHoveredColumn ) && table->HoveredColumnBody != -1
	...
	if ( table->IsSortSpecsDirty && ( table->Flags & ImGuiTableFlags_Sortable ) ) TableSortSpecsBuild( table );
...
void ImGui::TableUpdateBorders( ImGuiTable* table )
{
	...
	for ( int order_n = 0; order_n < table->ColumnsCount; order_n++ )
	{
		...
		const int column_n = table->DisplayOrderToIndex[ order_n ];
		ImGuiTableColumn* column = &table->Columns[ column_n ];
		if ( column->Flags & ( ImGuiTableColumnFlags_NoResize | ImGuiTableColumnFlags_NoDirectResize_ ) ) continue;

		// ImGuiTableFlags_NoBordersInBodyUntilResize will be honored in TableDrawBorders()
		const float border_y2_hit = ( table->Flags & ImGuiTableFlags_NoBordersInBody ) ? hit_y2_head : hit_y2_body;
		if ( ( table->Flags & ImGuiTableFlags_NoBordersInBody ) && table->IsUsingHeaders == false ) continue;
		...
		bool pressed = ButtonBehavior(
			...
			&hovered,
			&held,
			ImGuiButtonFlags_FlattenChildren | ImGuiButtonFlags_PressedOnClick | ImGuiButtonFlags_PressedOnDoubleClick
...
void ImGui::EndTable()
{
	...
	if ( table->Flags & ImGuiTableFlags_ScrollX )
	{
		...
		if ( table->RightMostEnabledColumn != -1 )
			max_pos_x = ImMax(
				max_pos_x,
				table->Columns[ table->RightMostEnabledColumn ].WorkMaxX + table->CellPaddingX + table->OuterPaddingX
	...
	if ( ( table->Flags & ImGuiTableFlags_ScrollX ) == 0 && inner_window != outer_window )
	{
		...
		table->LastResizedColumn != -1 && table->ResizedColumn == -1 && inner_window->ScrollbarX
		&& table->InstanceInteracted == table->InstanceCurrent )
	{
		...
		if ( column->MaxX < table->InnerClipRect.Min.x )
			SetScrollFromPosX(
				inner_window, column->MaxX - inner_window->Pos.x - neighbor_width_to_keep_visible, 1.0f );
	...
	IM_ASSERT_USER_ERROR(
		inner_window->IDStack.back() == table_instance->TableInstanceID, "Mismatching PushID/PopID!" );
	IM_ASSERT_USER_ERROR(
		outer_window->DC.ItemWidthStack.Size >= temp_data->HostBackupItemWidthStackSize, "Too many PopItemWidth!" );
	...
	else if ( temp_data->UserOuterSize.x <= 0.0f )
	{
		...
		outer_window->DC.IdealMaxPos.x = ImMax(
			...
			table->OuterRect.Min.x + table->ColumnsAutoFitWidth + decoration_size - temp_data->UserOuterSize.x );
...
void ImGui::
	TableSetupColumn( const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id )
{
	ImGuiContext& g = *GImGui;
	ImGuiTable* table = g.CurrentTable;
	...
	IM_ASSERT(
		( flags & ImGuiTableColumnFlags_StatusMask_ ) == 0
		&& "Illegal to pass StatusMask values to TableSetupColumn()" );
	if ( table->DeclColumnsCount >= table->ColumnsCount )
	{
		IM_ASSERT_USER_ERROR(
			table->DeclColumnsCount < table->ColumnsCount, "Called TableSetupColumn() too many times!" );
		return;
	}
...
void ImGui::TableBeginRow( ImGuiTable* table )
{
	...
	window->DC.PrevLineSize = window->DC.CurrLineSize = ImVec2(
		0.0f,
		0.0f ); // This allows users to call SameLine() to share LineSize between columns, and to call it from first column too.
	...
	if ( is_visible )
	{
		...
		if ( top_border_col && bg_y1 >= table->BgClipRect.Min.y && bg_y1 < table->BgClipRect.Max.y )
			window->DrawList->AddLine(
				ImVec2( table->BorderX1, bg_y1 ), ImVec2( table->BorderX2, bg_y1 ), top_border_col, border_size );
...
void ImGui::TableMergeDrawChannels( ImGuiTable* table )
{
	...
	if ( merge_group_mask != 0 )
	{
		...
		memcpy(
			splitter->_Channels.Data + LEADING_DRAW_CHANNELS,
...
void ImGui::TableDrawBorders( ImGuiTable* table )
{
	...
	const float draw_y2_head = table->IsUsingHeaders ? ImMin(
								   table->InnerRect.Max.y,
								   ( table->FreezeRowsCount >= 1 ? table->InnerRect.Min.y : table->WorkRect.Min.y )
	...
	if ( table->Flags & ImGuiTableFlags_BordersOuter )
	{
		...
		{
			inner_drawlist
				->AddRect( outer_border.Min, outer_border.Max + ImVec2( 1, 1 ), outer_col, 0.0f, 0, border_size );
		}
...
void ImGui::TableSetColumnSortDirection( int column_n, ImGuiSortDirection sort_direction, bool append_to_sort_specs )
{
	...
	if ( !( table->Flags & ImGuiTableFlags_SortTristate ) ) IM_ASSERT( sort_direction != ImGuiSortDirection_None );
...
void ImGui::TableHeader( const char* label )
{
	...
	if ( text_clipped && hovered && g.ActiveId == 0 ) SetItemTooltip( "%.*s", (int)( label_end - label ), label );
...
void ImGui::TableAngledHeadersRowEx( float angle, float max_label_width )
{
	...
	const ImRect row_r(
		table->WorkRect.Min.x, table->BgClipRect.Min.y, table->WorkRect.Max.x, window->DC.CursorPos.y + row_height );
		...
		for ( int order_n = 0; order_n < table->ColumnsCount; order_n++ )
		{
			...
			if ( pass == 0 )
			{
				...
				draw_list->AddQuadFilled(
					bg_shape[ 0 ], bg_shape[ 1 ], bg_shape[ 2 ], bg_shape[ 3 ], GetColorU32( ImGuiCol_TableHeaderBg ) );
				if ( column_n == highlight_column_n )
					draw_list->AddQuadFilled(
						...
						bg_shape[ 2 ],
						bg_shape[ 3 ],
						GetColorU32( ImGuiCol_Header ) ); // Highlight on hover
				...
				ImRect label_r(
					...
					window->ClipRect.Min
						+ ImVec2(
							clip_width + ( flip_label ? 0.0f : table->CellPaddingX ),
							header_height + table->RowCellPaddingY ) );
				ImVec2 label_size = CalcTextSize( label_name, NULL, true );
				ImVec2 label_off = ImVec2(
					flip_label ? ImMax( 0.0f, max_label_width - label_size.x - table->CellPaddingX ) :
								 table->CellPaddingX,
					table->RowCellPaddingY );
				int vtx_idx_begin = draw_list->_VtxCurrentIdx;
				RenderTextEllipsis(
					draw_list,
					label_r.Min + label_off,
					label_r.Max,
					label_r.Max.x,
					label_r.Max.x,
					label_name,
				...
				ImVec2 pivot_out = ImVec2( column->WorkMinX, row_r.Max.y )
				                 + ( flip_label ? ( unit_right * clip_width ) : ImVec2( header_height, 0.0f ) );
...
void ImGui::TableSaveSettings( ImGuiTable* table )
{
	...
	for ( int n = 0; n < table->ColumnsCount; n++, column++, column_settings++ )
	{
		...
		if ( width_or_weight != column->InitStretchWeightOrWidth ) settings->SaveFlags |= ImGuiTableFlags_Resizable;
...
void ImGui::TableGcCompactSettings()
{
	...
	for ( ImGuiTableSettings* settings = g.SettingsTables.begin(); settings != NULL;
	      settings = g.SettingsTables.next_chunk( settings ) )
		if ( settings->ID != 0 ) required_memory += (int)TableSettingsCalcChunkSize( settings->ColumnsCount );
	...
	for ( ImGuiTableSettings* settings = g.SettingsTables.begin(); settings != NULL;
		...
		if ( settings->ID != 0 )
			memcpy(
				new_chunk_stream.alloc_chunk( TableSettingsCalcChunkSize( settings->ColumnsCount ) ),
				settings,
...
void ImGui::DebugNodeTable( ImGuiTable* table )
{
	...
	BulletText(
		"ResizedColumn: %d, ReorderColumn: %d, HeldHeaderColumn: %d",
		table->ResizedColumn,
		table->ReorderColumn,
...
{
	if ( !TreeNode(
			 (void*)(intptr_t)settings->ID, "Settings 0x%08X (%d columns)", settings->ID, settings->ColumnsCount ) )
		return;
...
void ImGui::BeginColumns( const char* str_id, int columns_count, ImGuiOldColumnFlags flags )
{
	...
	const float max_1 =
		window->WorkRect.Max.x + column_padding - ImMax( column_padding - window->WindowPadding.x, 0.0f );
