	...
	const int iR_in )
{
	...
	if ( fSep >= fvMax[ channel ] || fSep <= fvMin[ channel ] )
	{
		...
		for ( l = iL_in; l <= iR_in; l++ )
		{
			...
			while ( l2 < l && bNotFound )
			{
				...
				if ( vP.x == vP2.x && vP.y == vP2.y && vP.z == vP2.z && vN.x == vN2.x && vN.y == vN2.y && vN.z == vN2.z
	...
	else
	{
		...
		if ( iL < iR_in )
			MergeVertsFast(
				piTriList_in_and_out, pTmpVert, pContext, iL, iR_in ); // weld all right of (or equal to) fSep
	}
}
	...
	STriInfo pTriInfos[], int piTriList_out[], const SMikkTSpaceContext* pContext, const int iNrTrianglesIn )
{
	...
	for ( t = 0; t < iNrTrianglesIn; t++ ) pTriInfos[ t ].iFlag = 0;
...
static tbool AssignRecur( const int piTriListIn[], STriInfo psTriInfos[], const int iMyTriIndex, SGroup* pGroup )
{
...

	{
		...
		if ( neigh_indexL >= 0 ) AssignRecur( piTriListIn, psTriInfos, neigh_indexL, pGroup );
		if ( neigh_indexR >= 0 ) AssignRecur( piTriListIn, psTriInfos, neigh_indexR, pGroup );
	}
		...
		for ( i = 0; i < pGroup->iNrFaces; i++ ) // triangles
		{
			...
			if ( !bFound )
			{
				...
				if ( pIndices == NULL )
				{
					// clean up and return false
					int s = 0;
					for ( s = 0; s < iUniqueSubGroups; s++ ) free( pUniSubGroups[ s ].pTriMembers );
					free( pUniSubGroups );
					free( pTmpMembers );
...
static void QuickSort( int* pSortBuffer, int iLeft, int iRight, unsigned int uSeed )
{
	...
	if ( iLeft < iR ) QuickSort( pSortBuffer, iLeft, iR, uSeed );
	if ( iL < iRight ) QuickSort( pSortBuffer, iL, iRight, uSeed );
}
...
static void QuickSortEdges( SEdge* pSortBuffer, int iLeft, int iRight, const int channel, unsigned int uSeed )
{
	...
	do {
		...
		while ( pSortBuffer[ iR ].array[ channel ] > iMid ) --iR;
	...
	if ( iLeft < iR ) QuickSortEdges( pSortBuffer, iLeft, iR, channel, uSeed );
	if ( iL < iRight ) QuickSortEdges( pSortBuffer, iL, iRight, channel, uSeed );
}
...
static void DegenPrologue( STriInfo pTriInfos[], int piTriList_out[], const int iNrTrianglesIn, const int iTotTris )
{
	...
	while ( t < iNrTrianglesIn && bStillFindingGoodOnes )
	{
		...
		if ( bIsGood )
		{
			if ( iNextGoodTriangleSearchIndex < ( t + 2 ) ) iNextGoodTriangleSearchIndex = t + 2;
		}
		else
		{
			int t0, t1;
