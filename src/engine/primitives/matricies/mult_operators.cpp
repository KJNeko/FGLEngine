...
namespace fgl::engine
{
	...
	{
		return NormalVector( static_cast< glm::mat4 >( rot_mat ) * glm::vec4( static_cast< glm::vec3 >( vec ), 0.0f ) );
	}

	Vector operator*( const RotationMatrix rot_mat, const Vector vec )
	{
		return Vector( static_cast< glm::mat4 >( rot_mat ) * glm::vec4( static_cast< glm::vec3 >( vec ), 0.0f ) );
	}
