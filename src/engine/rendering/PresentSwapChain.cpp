...
namespace fgl::engine
{
	...
	void PresentSwapChain::createSyncObjects()
	{
		...
		for ( size_t i = 0; i < imageCount(); i++ )
		{
			...
			{
				vk::DebugUtilsObjectNameInfoEXT debug_info {};
				debug_info.objectType = vk::ObjectType::eSemaphore;
				debug_info.objectHandle =
					reinterpret_cast< uint64_t >( static_cast< VkSemaphore >( *m_image_available_sem[ i ] ) );
				const std::string name { std::format( "Image available semaphore: {}", i ) };
				debug_info.pObjectName = name.c_str();
				Device::getInstance()->setDebugUtilsObjectNameEXT( debug_info );
			}
...

			{
				...
				debug_info.objectType = vk::ObjectType::eSemaphore;
				debug_info.objectHandle =
					reinterpret_cast< uint64_t >( static_cast< VkSemaphore >( *m_render_finished_sem[ i ] ) );
				const std::string name { std::format( "Render finished semaphore: {}", i ) };
				debug_info.pObjectName = name.c_str();
				Device::getInstance()->setDebugUtilsObjectNameEXT( debug_info );
			}
